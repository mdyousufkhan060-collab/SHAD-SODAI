...
interface AdminPromoBannersProps {
  language: 'en' | 'bn';
  onEdit: (banner?: Banner) => void;
}

const AdminPromoBanners: React.FC<AdminPromoBannersProps> = ({ language, onEdit }) => {
  ...
  const fetchBanners = async () => {
    ...
    setError(null);
    try {
      const response = await fetch('/api/admin/banners?location=homepage_promo', {
    ...
    <div className="animate-fade-in text-left">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
        <div className="space-y-2">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-4xl font-black text-gray-900 tracking-tight">
              {language === 'bn' ? 'প্রোমোশনাল ব্যানার' : 'Promotional Banners'}
            </h1>
          </div>
          <p className="text-gray-500 text-sm font-semibold">
            {language === 'bn' ? 'হোমপেজের প্রোমোশনাল ব্যানারগুলো ম্যানেজ করুন।' : 'Manage middle-of-page promotional banners for the homepage.'}
          </p>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          >
            ...
            <span>{language === 'bn' ? 'নতুন ব্যানার যোগ করুন' : 'ADD PROMO BANNER'}</span>
          </button>
          <button 
      ...
      <div className="flex flex-col md:flex-row gap-6 items-center mb-12">
        <div className="relative flex-1 w-full">
          ...
          <input 
            type="text" 
            placeholder={language === 'bn' ? 'ব্যানার নাম দিয়ে খুঁজুন...' : 'Search banners...'}
            value={searchTerm}
      ...
      <div className="space-y-8">
        {isLoading ? (
          ...
          <div className="py-32 text-center space-y-6 bg-gray-50 rounded-[40px] border-2 border-dashed border-gray-100">
            ...
            <div className="space-y-2">
              <h3 className="text-xl font-black text-gray-800">{language === 'bn' ? 'কোনো ব্যানার পাওয়া যায়নি' : 'No Promo Banners Found'}</h3>
              <p className="text-sm text-gray-400 font-bold uppercase tracking-wider">
                {searchTerm || statusFilter !== 'all' 
                  ? (language === 'bn' ? 'আপনার ফিল্টার পরিবর্তন করে দেখুন।' : 'Try adjusting your search or filters.')
                  : (language === 'bn' ? 'নতুন ব্যানার যোগ করতে উপরের বোতামটি ক্লিক করুন।' : 'Start by adding your first promo banner.')}
              </p>
            </div>
          </div>
        ) : (
            ...
            {filteredBanners.map((banner) => (
              ...
              >
                ...
                <div className="relative w-full md:w-64 aspect-video bg-gray-100 rounded-[24px] overflow-hidden border border-gray-150 shadow-sm group-hover:border-emerald-300 transition-all">
                  <img 
                    src={banner.image_url_desktop || 'https://placehold.co/1920x400?text=No+Image'} 
                    alt={banner.name} 
                    className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-110"
                    onError={(e) => { (e.target as any).src = 'https://placehold.co/1920x400?text=No+Image'; }}
                  />
