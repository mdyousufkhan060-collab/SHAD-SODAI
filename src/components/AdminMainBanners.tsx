...
interface AdminMainBannersProps {
  language: 'en' | 'bn';
  onEdit: (banner?: Banner) => void;
}

const AdminMainBanners: React.FC<AdminMainBannersProps> = ({ language, onEdit }) => {
  ...
  const fetchBanners = async () => {
    ...
    setError(null);
    try {
      const response = await fetch('/api/admin/banners?location=homepage_hero', {
  ...
  const filteredBanners = banners.filter(b => {
    const matchesSearch = b.name.toLowerCase().includes(searchTerm.toLowerCase()) || 
                          (b.heading_en || '').toLowerCase().includes(searchTerm.toLowerCase());
    ...
    <div className="animate-fade-in text-left">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
        <div className="space-y-2">
          <h1 className="text-4xl font-black text-gray-900 tracking-tight">
            {language === 'bn' ? 'মূল / হোম ব্যানার' : 'Main / Home Banners'}
          </h1>
          <p className="text-gray-500 text-sm font-semibold">
            {language === 'bn' ? 'হোমপেজের মূল ব্যানারগুলো ম্যানেজ করুন এবং পরিবর্তন করুন সরাসরি ডাটাবেস থেকে।' : 'Manage hero and promotional banners for the homepage directly from the database server.'}
          </p>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          >
            ...
            <span>{language === 'bn' ? 'নতুন ব্যানার যোগ করুন' : 'ADD NEW BANNER'}</span>
          </button>
          <button 
      ...
      <div className="flex flex-col md:flex-row gap-6 items-center mb-12">
        <div className="relative flex-1 w-full">
          ...
          <input 
            type="text" 
            placeholder={language === 'bn' ? 'ব্যানার নাম বা হেডিং দিয়ে খুঁজুন...' : 'Search banners by name or heading...'}
            value={searchTerm}
      ...
      <div className="space-y-8">
        {isLoading ? (
          ...
          <div className="py-32 text-center space-y-6 bg-gray-50 rounded-[40px] border-2 border-dashed border-gray-100">
            ...
            <div className="space-y-2">
              <h3 className="text-xl font-black text-gray-800">{language === 'bn' ? 'কোনো ব্যানার পাওয়া যায়নি' : 'No Banners Found'}</h3>
              <p className="text-sm text-gray-400 font-bold uppercase tracking-wider">
                {searchTerm || statusFilter !== 'all' 
                  ? (language === 'bn' ? 'আপনার ফিল্টার পরিবর্তন করে দেখুন।' : 'Try adjusting your search or filters.')
                  : (language === 'bn' ? 'নতুন ব্যানার যোগ করতে উপরের বোতামটি ক্লিক করুন।' : 'Start by adding your first banner.')}
              </p>
            </div>
          </div>
        ) : (
            ...
            {filteredBanners.map((banner) => (
              ...
              >
                ...
                <div className="relative w-full md:w-64 aspect-video bg-gray-100 rounded-[24px] overflow-hidden border border-gray-150 shadow-sm group-hover:border-emerald-300 transition-all">
                  <img 
                    src={banner.image_url_desktop || 'https://placehold.co/1920x700?text=No+Image'} 
                    alt={banner.name} 
                    className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-110"
                    onError={(e) => { (e.target as any).src = 'https://placehold.co/1920x700?text=No+Image'; }}
                  />
                ...
                <div className="flex-1 space-y-3 text-center md:text-left w-full">
                  ...
                  <div className="flex items-center justify-center md:justify-start gap-3">
                    <div className="flex items-center gap-2 text-[10px] font-bold text-emerald-700 bg-emerald-50 px-3 py-1 rounded-xl border border-emerald-100">
                      <ExternalLink className="w-3 h-3" />
                      <span className="truncate max-w-[200px]">{banner.button_link || 'No Destination Link'}</span>
                    </div>
                  </div>
                </div>
