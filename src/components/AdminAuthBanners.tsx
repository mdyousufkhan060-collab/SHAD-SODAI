...
interface AdminAuthBannersProps {
  language: 'en' | 'bn';
  onEdit: (banner?: Banner) => void;
}

const AdminAuthBanners: React.FC<AdminAuthBannersProps> = ({ language, onEdit }) => {
  ...
  const fetchBanners = async () => {
    ...
    setError(null);
    try {
      const response = await fetch('/api/admin/banners?location=auth_banner', {
    ...
    <div className="animate-fade-in text-left">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
        <div className="space-y-2">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-4xl font-black text-gray-900 tracking-tight">
              {language === 'bn' ? 'লগইন / অ্যাকাউন্ট ব্যানার' : 'Login / Account Banners'}
            </h1>
          </div>
          <p className="text-gray-500 text-sm font-semibold">
            {language === 'bn' ? 'লগইন ও রেজিস্ট্রেশন পেজের ব্যানারগুলো ম্যানেজ করুন এবং অ্যাকাউন্ট সেকশন কাস্টমাইজ করুন।' : 'Manage promotional banners for login and registration pages and customize the account section.'}
          </p>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          >
            ...
            <span>{language === 'bn' ? 'নতুন অথ ব্যানার' : 'ADD AUTH BANNER'}</span>
          </button>
          <button 
      ...
      <div className="flex flex-col md:flex-row gap-6 items-center mb-12">
        <div className="relative flex-1 w-full">
          ...
          <input 
            type="text" 
            placeholder={language === 'bn' ? 'ব্যানার নাম দিয়ে খুঁজুন...' : 'Search banners by name...'}
            value={searchTerm}
      ...
      <div className="space-y-8">
        {isLoading ? (
          <div className="py-32 flex flex-col items-center justify-center space-y-6">
            ...
            <p className="text-gray-400 text-xs font-black tracking-widest uppercase">{language === 'bn' ? 'লোড হচ্ছে...' : 'QUERYING DATA...'}</p>
          </div>
          ...
          <div className="py-32 text-center space-y-6 bg-gray-50 rounded-[40px] border-2 border-dashed border-gray-100">
            ...
            <div className="space-y-2">
              <h3 className="text-xl font-black text-gray-800">{language === 'bn' ? 'কোনো ব্যানার পাওয়া যায়নি' : 'No Auth Banners'}</h3>
              <p className="text-sm text-gray-400 font-bold uppercase tracking-wider">
                {language === 'bn' ? 'নতুন অথ ব্যানার যোগ করতে উপরের বোতামটি ক্লিক করুন।' : 'Start by adding your first auth banner.'}
              </p>
            </div>
          </div>
        ) : (
            ...
            {filteredBanners.map((banner) => (
              ...
              >
                ...
                <div className="relative w-full md:w-64 aspect-video bg-gray-100 rounded-[24px] overflow-hidden border border-gray-150 shadow-sm group-hover:border-emerald-300 transition-all">
                  <img 
                    src={banner.image_url_desktop || 'https://placehold.co/800x400?text=No+Image'} 
                    alt={banner.name} 
                    className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-110"
                    onError={(e) => { (e.target as any).src = 'https://placehold.co/800x400?text=No+Image'; }}
                  />
...
export default AdminAuthBanners;
